...
有时，什么代码该写进controller，什么代码该写进model是容易混淆的。最佳的实践是，永远不要在controller里直接调用数据库，应该使用model提供方法来代替之。例如，如果你有一个汽车model，然后想要在某辆车上安上四个轮子，你不应该直接调用db.update(id, {wheels: 4})，而是应该调用类似car.mountWheels(id, 4)这样的model方法。
